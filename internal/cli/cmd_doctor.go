...
func cmdDoctor(w, wErr io.Writer, gf GlobalFlags, args []string, version string) int {
	const usage = "Usage: tumux doctor [--json]"
	if len(args) > 0 {
	...
	// 3. tumux home exists and writable
...
func checkHomeDir(home string) checkResult {
	...
	if err != nil {
		return checkResult{Name: "tumux_home", Status: "fail", Message: home + " not found"}
	}
	if !info.IsDir() {
		return checkResult{Name: "tumux_home", Status: "fail", Message: home + " is not a directory"}
	}
	// Check writable by creating a temp file
	tmp, err := os.CreateTemp(home, ".doctor-check-*")
	if err != nil {
		return checkResult{Name: "tumux_home", Status: "warn", Message: home + " is not writable"}
	}
	_ = tmp.Close()
	_ = os.Remove(tmp.Name())
	return checkResult{Name: "tumux_home", Status: "ok", Message: home}
}
