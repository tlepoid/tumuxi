...
func TestOpenClawPresentScript_AugmentsChannelEnvelope(t *testing.T) {
	...
	scriptPath := filepath.Join("..", "..", "skills", "tumux", "scripts", "openclaw-present.sh")
	env := withEnv(os.Environ(), "OPENCLAW_CHANNEL", "msteams")
	input := `{"ok":true,"summary":"ok","message":"Build complete","quick_actions":[{"id":"status","label":"Status","command":"tumux --json status","style":"primary","prompt":"Check status"}],"channel":{"message":"Build complete","chunks_meta":[{"index":1,"total":1,"text":"Build complete"}]}}`
	...
	if got, _ := actionMap["status"].(string); got != "tumux --json status" {
		t.Fatalf("quick_action_by_id[status] = %q, want %q", got, "tumux --json status")
	}
}
