...
func DefaultOptions() Options {
	server := strings.TrimSpace(os.Getenv("TUMUX_TMUX_SERVER"))
	if server == "" {
		server = "tumux"
	}
	config := strings.TrimSpace(os.Getenv("TUMUX_TMUX_CONFIG"))
	if config == "" {
		config = "/dev/null"
	}
	return Options{
...
// which can cause commands aimed at "tumux-ws-tab-1" to hit "tumux-ws-tab-10".
...
// Bare names are safe here because tumux session names include workspace ID +
