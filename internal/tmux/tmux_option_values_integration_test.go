...
func TestSetGlobalOptionValues_SetsGlobalOptions(t *testing.T) {
	...
	if err := SetGlobalOptionValues([]OptionValue{
		{Key: "@tumux_batch_opt_a", Value: "a"},
		{Key: "@tumux_batch_opt_b", Value: "b"},
	}, opts); err != nil {
		t.Fatalf("SetGlobalOptionValues: %v", err)
	}

	gotA, err := GlobalOptionValue("@tumux_batch_opt_a", opts)
	if err != nil {
		t.Fatalf("GlobalOptionValue @tumux_batch_opt_a: %v", err)
	}
	if gotA != "a" {
		t.Fatalf("expected @tumux_batch_opt_a=%q, got %q", "a", gotA)
	}

	gotB, err := GlobalOptionValue("@tumux_batch_opt_b", opts)
	if err != nil {
		t.Fatalf("GlobalOptionValue @tumux_batch_opt_b: %v", err)
	}
	if gotB != "b" {
		t.Fatalf("expected @tumux_batch_opt_b=%q, got %q", "b", gotB)
	}
}
...
func TestGlobalOptionValues_ReadsMultipleOptions(t *testing.T) {
	...
	if err := SetGlobalOptionValues([]OptionValue{
		{Key: "@tumux_batch_read_a", Value: "read-a"},
		{Key: "@tumux_batch_read_b", Value: "read-b"},
	}, opts); err != nil {
		t.Fatalf("SetGlobalOptionValues: %v", err)
	}
	...
	values, err := GlobalOptionValues([]string{
		"@tumux_batch_read_a",
		"@tumux_batch_read_b",
		"@tumux_batch_read_missing",
	}, opts)
	if err != nil {
		t.Fatalf("GlobalOptionValues: %v", err)
	}
	if got := values["@tumux_batch_read_a"]; got != "read-a" {
		t.Fatalf("expected @tumux_batch_read_a=%q, got %q", "read-a", got)
	}
	if got := values["@tumux_batch_read_b"]; got != "read-b" {
		t.Fatalf("expected @tumux_batch_read_b=%q, got %q", "read-b", got)
	}
	if got := values["@tumux_batch_read_missing"]; got != "" {
