...
func soakDuration(t *testing.T, fallback time.Duration) time.Duration {
	t.Helper()
	raw := os.Getenv("TUMUX_SOAK_DURATION")
	if raw == "" {
		if mins := os.Getenv("TUMUX_SOAK_MINUTES"); mins != "" {
			val, err := strconv.Atoi(mins)
			if err != nil || val <= 0 {
				t.Fatalf("invalid TUMUX_SOAK_MINUTES=%q", mins)
			}
	...
	if err != nil || d <= 0 {
		t.Fatalf("invalid TUMUX_SOAK_DURATION=%q", raw)
	}
	return d
}
