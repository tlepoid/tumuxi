...
func TestEnvBuilder_BuildEnv(t *testing.T) {
	...
	wt := &data.Workspace{
		...
		Branch: "feature-1",
		Repo:   "/home/user/repo",
		Root:   "/home/user/.tumux/workspaces/feature-1",
	...
	checks := map[string]string{
		"TUMUX_WORKSPACE_NAME":   "feature-1",
		"TUMUX_WORKSPACE_ROOT":   "/home/user/.tumux/workspaces/feature-1",
		"TUMUX_WORKSPACE_BRANCH": "feature-1",
	...
	portFound := false
	for _, e := range env {
		if strings.HasPrefix(e, "TUMUX_PORT=") {
			portFound = true
			break
		}
	}
	if !portFound {
		t.Error("Missing TUMUX_PORT env var")
	}
}
...
func TestEnvBuilder_BuildEnvMap(t *testing.T) {
	...
	if envMap["TUMUX_WORKSPACE_NAME"] != "feature-1" {
		t.Errorf("TUMUX_WORKSPACE_NAME = %v, want feature-1", envMap["TUMUX_WORKSPACE_NAME"])
	}
	if envMap["TUMUX_PORT"] != "6200" {
		t.Errorf("TUMUX_PORT = %v, want 6200", envMap["TUMUX_PORT"])
	}
}
...
func TestEnvBuilder_NilPortAllocator(t *testing.T) {
	...
	for _, e := range env {
		if strings.HasPrefix(e, "TUMUX_PORT=") {
			t.Error("Should not have TUMUX_PORT with nil allocator")
		}
	}
}
