#!/usr/bin/env bash
# openclaw-dx.sh — OpenClaw-first control plane for tumux coding workflows.
#
...
TUMUX_ERROR_OUTPUT=""
TUMUX_ERROR_CAPTURE_FILE=""
if TUMUX_ERROR_CAPTURE_FILE="$(mktemp "${TMPDIR:-/tmp}/tumux-openclaw-dx-error.XXXXXX" 2>/dev/null)"; then
  :
else
  TUMUX_ERROR_CAPTURE_FILE="${TMPDIR:-/tmp}/tumux-openclaw-dx-error.$$"
fi
_openclaw_dx_cleanup() {
  if [[ -n "${TUMUX_ERROR_CAPTURE_FILE:-}" && -f "$TUMUX_ERROR_CAPTURE_FILE" ]]; then
    rm -f "$TUMUX_ERROR_CAPTURE_FILE" 2>/dev/null || true
  fi
}
...
tumux_ok_json() {
  local out
  TUMUX_ERROR_OUTPUT=""
  if [[ -n "${TUMUX_ERROR_CAPTURE_FILE:-}" ]]; then
    : >"$TUMUX_ERROR_CAPTURE_FILE" 2>/dev/null || true
  fi
  if ! out="$(tumux --json "$@" 2>&1)"; then
    TUMUX_ERROR_OUTPUT="$out"
    if [[ -n "${TUMUX_ERROR_CAPTURE_FILE:-}" ]]; then
      printf '%s' "$out" >"$TUMUX_ERROR_CAPTURE_FILE" 2>/dev/null || true
    fi
    return 1
  fi
...
DX_CMD_REF="skills/tumux/scripts/openclaw-dx.sh"
TURN_CMD_REF="skills/tumux/scripts/openclaw-turn.sh"
STEP_CMD_REF="skills/tumux/scripts/openclaw-step.sh"

normalize_command_refs() {
  local value="$1"
  value="${value//skills\/tumux\/scripts\/openclaw-dx.sh/$DX_CMD_REF}"
  value="${value//skills\/tumux\/scripts\/openclaw-turn.sh/$TURN_CMD_REF}"
  value="${value//skills\/tumux\/scripts\/openclaw-step.sh/$STEP_CMD_REF}"
  printf '%s' "$value"
}
...
emit_result() {
  ...
  quick_actions_json="$(jq -c --arg dx "$DX_CMD_REF" --arg turn "$TURN_CMD_REF" --arg step "$STEP_CMD_REF" '
    map(
      .command = ((.command // "")
        | gsub("skills/tumux/scripts/openclaw-dx\\.sh"; $dx)
        | gsub("skills/tumux/scripts/openclaw-turn\\.sh"; $turn)
        | gsub("skills/tumux/scripts/openclaw-step\\.sh"; $step)
      )
    )
  ' <<<"$quick_actions_json")"
...
emit_tumux_error() {
  local command_name="$1"
  local out="${2:-$TUMUX_ERROR_OUTPUT}"
  if [[ -z "${out// }" ]] && [[ -n "${TUMUX_ERROR_CAPTURE_FILE:-}" ]] && [[ -f "$TUMUX_ERROR_CAPTURE_FILE" ]]; then
    out="$(cat "$TUMUX_ERROR_CAPTURE_FILE" 2>/dev/null || true)"
  fi
  local err_code="command_error"
  local err_msg="tumux command failed"
  local err_details='{}'
  if jq -e . >/dev/null 2>&1 <<<"$out"; then
    err_code="$(jq -r '.error.code // "command_error"' <<<"$out")"
    err_msg="$(jq -r '.error.message // "tumux command failed"' <<<"$out")"
  ...
  RESULT_NEXT_ACTION="Fix the failing tumux command input and retry."
  RESULT_SUGGESTED_COMMAND=""
...
workspace_row_by_id() {
  ...
  local ws_out
  if ! ws_out="$(tumux_ok_json workspace list --archived)"; then
    return 1
  fi
...
agent_for_workspace() {
  local workspace_id="$1"
  local agents_out
  if ! agents_out="$(tumux_ok_json agent list --workspace "$workspace_id")"; then
    printf ''
    return 0
  fi
  ...
  while IFS=$'\t' read -r session_name agent_id; do
    ...
    if ! capture_out="$(tumux_ok_json agent capture "$session_name" --lines 48)"; then
      continue
    fi
...
context_file_path() {
  ...
  printf '%s' "$base/tumux/openclaw-dx-context.json"
}
...
project_row_by_path() {
  ...
  local canonical out
  canonical="$(canonicalize_path "$project_path")"
  if ! out="$(tumux_ok_json project list)"; then
    return 1
  fi
...
ensure_project_registered() {
  ...
  if ! add_out="$(tumux_ok_json project add "$project_path")"; then
    return 1
  fi
  jq -c '.data // {}' <<<"$add_out"
}
...
cmd_project_add() {
  ...
  local add_out
  if ! add_out="$(tumux_ok_json project add "$path")"; then
    emit_tumux_error "project.add"
    return
  fi
  ...
  if [[ -n "$workspace_name" ]]; then
    ...
    if ! ws_create_out="$(tumux_ok_json "${ws_args[@]}")"; then
      local err_payload err_code err_message retry_cmd
      err_payload="$TUMUX_ERROR_OUTPUT"
      if [[ -z "${err_payload// }" ]] && [[ -n "${TUMUX_ERROR_CAPTURE_FILE:-}" ]] && [[ -f "$TUMUX_ERROR_CAPTURE_FILE" ]]; then
        err_payload="$(cat "$TUMUX_ERROR_CAPTURE_FILE" 2>/dev/null || true)"
      fi
      err_code=""
      err_message=""
      ...
      if workspace_create_needs_initial_commit "$err_code" "$err_message"; then
        retry_cmd="skills/tumux/scripts/openclaw-dx.sh project add --path $(shell_quote "$project_path") --workspace $(shell_quote "$workspace_name") --assistant $(shell_quote "${assistant:-codex}")"
        if [[ -n "$base" ]]; then
      ...
      emit_tumux_error "project.add" "$err_payload"
      return
    fi
  ...
  RESULT_SUGGESTED_COMMAND=""
  if [[ -n "$workspace_id" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace_id") --assistant $(shell_quote "${assistant:-codex}") --prompt \"Analyze the biggest tech-debt items and fix the top one.\""
  else
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh workspace create --name mobile --project $(shell_quote "$project_path") --assistant codex"
  fi

  local actions='[]'
  actions="$(append_action "$actions" "ws_list" "Workspaces" "skills/tumux/scripts/openclaw-dx.sh workspace list --project $(shell_quote "$project_path")" "primary" "List workspaces for this project")"
  if [[ -z "$workspace_id" ]]; then
    actions="$(append_action "$actions" "ws_create" "Create WS" "skills/tumux/scripts/openclaw-dx.sh workspace create --name mobile --project $(shell_quote "$project_path") --assistant codex" "success" "Create a workspace for mobile coding")"
  else
    actions="$(append_action "$actions" "start" "Start" "skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace_id") --assistant $(shell_quote "${assistant:-codex}") --prompt \"Analyze technical debt and implement the highest-impact fix.\"" "success" "Start a coding turn in this workspace")"
  fi
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status" "primary" "Show global coding status")"
...
cmd_project_list() {
  ...
  if [[ "$count" -gt 0 ]]; then
    ...
    if [[ -n "$first_project_name" ]]; then
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh project pick --name $(shell_quote "$first_project_name")"
    else
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh project pick --index 1"
    fi
  elif [[ -n "${query// }" ]]; then
  ...
  local actions='[]'
  if [[ -n "$first_project_name" ]]; then
    actions="$(append_action "$actions" "pick1" "Pick #1" "skills/tumux/scripts/openclaw-dx.sh project pick --name $(shell_quote "$first_project_name")" "primary" "Select the first project")"
  fi
  if [[ -n "$first_project_path" ]]; then
    actions="$(append_action "$actions" "ws1" "WS #1" "skills/tumux/scripts/openclaw-dx.sh workspace list --project $(shell_quote "$first_project_path")" "primary" "List workspaces for project #1")"
  fi
  local page_cmd_base="skills/tumux/scripts/openclaw-dx.sh project list --limit $limit"
  if [[ -n "${query// }" ]]; then
  ...
  if [[ -n "${query// }" ]]; then
    actions="$(append_action "$actions" "clear_query" "Clear Query" "skills/tumux/scripts/openclaw-dx.sh project list" "primary" "Show all projects")"
  fi
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status" "primary" "Show global coding status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_project_pick() {
  ...
  if [[ -n "${name_query// }" ]]; then
    ...
    if [[ "$match_count" -gt 1 ]]; then
      ...
      while IFS= read -r row; do
        ...
        actions="$(append_action "$actions" "pick_${row_index}" "Pick #$row_index" "skills/tumux/scripts/openclaw-dx.sh project pick --index $row_index" "primary" "Select $row_name")"
      done < <(jq -c '.[0:6][]' <<<"$matches")
      actions="$(append_action "$actions" "list" "List" "skills/tumux/scripts/openclaw-dx.sh project list --query $(shell_quote "$name_query")" "primary" "Show filtered projects again")"
      RESULT_QUICK_ACTIONS="$actions"
  ...
  if [[ -z "$workspace_name" ]]; then
    ...
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh workspace create --name mobile --project $(shell_quote "$selected_path") --assistant codex"
    ...
    actions="$(append_action "$actions" "ws_list" "Workspaces" "skills/tumux/scripts/openclaw-dx.sh workspace list --project $(shell_quote "$selected_path")" "primary" "List project workspaces")"
    RESULT_QUICK_ACTIONS="$actions"
  ...
  if ! ws_out="$(tumux_ok_json "${ws_args[@]}")"; then
    emit_tumux_error "project.pick"
    return
  fi
  ...
  RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$ws_id") --assistant $(shell_quote "${assistant:-codex}") --prompt \"Analyze the biggest debt items and fix one high-impact issue.\""

  local actions='[]'
  actions="$(append_action "$actions" "start" "Start" "$RESULT_SUGGESTED_COMMAND" "success" "Start a coding turn")"
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$ws_id")" "primary" "Show workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_guide() {
  ...
  if ! projects_out="$(tumux_ok_json project list)"; then
    emit_tumux_error "guide"
    return
  fi
  ...
  if ! ws_out="$(tumux_ok_json workspace list)"; then
    emit_tumux_error "guide"
    return
  fi
  ...
  if ! agents_out="$(tumux_ok_json agent list)"; then
    emit_tumux_error "guide"
    return
  fi
  agents_json="$(jq -c '.data // []' <<<"$agents_out")"
  ...
  if ! terms_out="$(tumux_ok_json terminal list)"; then
    terms_json='[]'
  else
  ...
  if [[ -n "$primary_session" ]]; then
    local capture_out
    if capture_out="$(tumux_ok_json agent capture "$primary_session" --lines "$capture_lines")"; then
  ...
  if [[ -z "$selected_project_path" ]]; then
    if [[ "$project_count" -eq 0 ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh project add --cwd --workspace mobile --assistant $(shell_quote "$assistant")"
    else
      ...
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh project list"
    fi
    ...
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh workspace decide --project $(shell_quote "$selected_project_path") --task $(shell_quote "$kickoff_prompt") --assistant $(shell_quote "$assistant")"
    ...
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh workspace list --project $(shell_quote "$selected_project_path")"
    ...
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$selected_workspace_id") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
    ...
    if [[ -n "$primary_agent" ]]; then
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$primary_agent") --text $(shell_quote "${capture_hint:-Continue with the safest option and report status plus next action.}") --enter"
    else
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh continue --workspace $(shell_quote "$selected_workspace_id") --text $(shell_quote "${capture_hint:-Continue with the safest option and report status plus next action.}") --enter"
    fi
    ...
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$selected_workspace_id") --assistant codex"
  else
    ...
    if [[ -n "$primary_agent" ]]; then
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$primary_agent") --text \"Continue from current state and report status plus next action.\" --enter"
    else
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh continue --workspace $(shell_quote "$selected_workspace_id") --text \"Continue from current state and report status plus next action.\" --enter"
    fi
  fi
  ...
  case "$stage" in
    add_project)
      actions="$(append_action "$actions" "add_cwd" "Add Project" "skills/tumux/scripts/openclaw-dx.sh project add --cwd --workspace mobile --assistant $(shell_quote "$assistant")" "success" "Register current directory and create a workspace")"
      actions="$(append_action "$actions" "project_list" "Projects" "skills/tumux/scripts/openclaw-dx.sh project list" "primary" "List registered projects")"
      ;;
    select_project)
      actions="$(append_action "$actions" "project_list" "Projects" "skills/tumux/scripts/openclaw-dx.sh project list" "primary" "List registered projects")"
      if [[ -n "$first_project_name" ]]; then
        actions="$(append_action "$actions" "pick_first" "Pick #1" "skills/tumux/scripts/openclaw-dx.sh project pick --name $(shell_quote "$first_project_name")" "success" "Select the first listed project")"
      fi
      ;;
    create_workspace)
      actions="$(append_action "$actions" "decide_ws" "Decide WS" "skills/tumux/scripts/openclaw-dx.sh workspace decide --project $(shell_quote "$selected_project_path") --task $(shell_quote "$kickoff_prompt") --assistant $(shell_quote "$assistant")" "success" "Get project vs nested workspace recommendation")"
      actions="$(append_action "$actions" "create_ws" "Create WS" "skills/tumux/scripts/openclaw-dx.sh workspace create --name mobile --project $(shell_quote "$selected_project_path") --assistant $(shell_quote "$assistant")" "primary" "Create a project workspace directly")"
      ;;
    select_workspace)
      actions="$(append_action "$actions" "list_ws" "Workspaces" "skills/tumux/scripts/openclaw-dx.sh workspace list --project $(shell_quote "$selected_project_path")" "primary" "List project workspaces")"
      if [[ -n "$first_workspace_id" ]]; then
        actions="$(append_action "$actions" "start_ws" "Start #1" "skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$first_workspace_id") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")" "success" "Start coding in the first workspace")"
      fi
      ;;
    start_agent)
      actions="$(append_action "$actions" "start" "Start" "skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$selected_workspace_id") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")" "success" "Start coding turn")"
      actions="$(append_action "$actions" "dual" "Dual Pass" "skills/tumux/scripts/openclaw-dx.sh workflow dual --workspace $(shell_quote "$selected_workspace_id") --implement-assistant claude --review-assistant codex" "primary" "Implement then review with separate assistants")"
      actions="$(append_action "$actions" "terminal" "Next.js Dev" "skills/tumux/scripts/openclaw-dx.sh terminal preset --workspace $(shell_quote "$selected_workspace_id") --kind nextjs" "primary" "Start Next.js dev server in this workspace")"
      ;;
    reply_agent)
      actions="$(append_action "$actions" "reply" "Reply" "$RESULT_SUGGESTED_COMMAND" "danger" "Reply to blocked agent prompt")"
      actions="$(append_action "$actions" "status_ws" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$selected_workspace_id")" "primary" "Check workspace state")"
      actions="$(append_action "$actions" "alerts_ws" "Alerts" "skills/tumux/scripts/openclaw-dx.sh alerts --workspace $(shell_quote "$selected_workspace_id")" "primary" "Show blocking alerts only")"
      ;;
    restart_agent)
      ...
      ;;
    review_and_ship)
      actions="$(append_action "$actions" "review" "Review" "skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$selected_workspace_id") --assistant codex" "success" "Review uncommitted changes")"
      actions="$(append_action "$actions" "ship" "Ship" "skills/tumux/scripts/openclaw-dx.sh git ship --workspace $(shell_quote "$selected_workspace_id")" "primary" "Commit current changes")"
      actions="$(append_action "$actions" "dual" "Dual Pass" "skills/tumux/scripts/openclaw-dx.sh workflow dual --workspace $(shell_quote "$selected_workspace_id") --implement-assistant claude --review-assistant codex" "primary" "Run implementation+review pass")"
      ;;
    continue_agent)
      actions="$(append_action "$actions" "continue" "Continue" "$RESULT_SUGGESTED_COMMAND" "success" "Continue active coding turn")"
      actions="$(append_action "$actions" "status_ws" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$selected_workspace_id")" "primary" "Check workspace state")"
      actions="$(append_action "$actions" "logs" "Terminal Logs" "skills/tumux/scripts/openclaw-dx.sh terminal logs --workspace $(shell_quote "$selected_workspace_id") --lines 120" "primary" "Inspect terminal output")"
      ;;
  esac

  if [[ -n "$selected_workspace_id" ]]; then
    actions="$(append_action "$actions" "cleanup" "Cleanup" "skills/tumux/scripts/openclaw-dx.sh cleanup --older-than 24h --yes" "primary" "Prune stale sessions")"
  elif [[ -n "$first_project_path" ]]; then
    actions="$(append_action "$actions" "workspace_list_first" "WS #1" "skills/tumux/scripts/openclaw-dx.sh workspace list --project $(shell_quote "$first_project_path")" "primary" "Inspect first project's workspaces")"
  fi
  actions="$(append_action "$actions" "status_global" "Global Status" "skills/tumux/scripts/openclaw-dx.sh status" "primary" "Show global status across projects")"
  RESULT_QUICK_ACTIONS="$actions"
...
workspace_create_emit_existing_recovery() {
  ...
  if ! ws_out="$(tumux_ok_json workspace list --repo "$project")"; then
    return 1
  fi
  ...
  RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$existing_id") --assistant $(shell_quote "$suggested_assistant") --prompt \"Summarize current status and continue with next high-impact task.\""
  ...
  actions="$(append_action "$actions" "list_ws" "Workspaces" "skills/tumux/scripts/openclaw-dx.sh workspace list --project $(shell_quote "$project")" "primary" "List all workspaces for this project")"
  actions="$(append_action "$actions" "retry_new_name" "New Name" "skills/tumux/scripts/openclaw-dx.sh workspace create --name $(shell_quote "$alt_name") --project $(shell_quote "$project") --scope $(shell_quote "$requested_scope") --assistant $(shell_quote "$suggested_assistant")" "primary" "Retry workspace creation with a new name")"
  RESULT_QUICK_ACTIONS="$actions"
...
emit_initial_commit_guidance() {
  ...
  actions="$(append_action "$actions" "project_only" "Project Only" "skills/tumux/scripts/openclaw-dx.sh project add --path $(shell_quote "$project")" "primary" "Register project without creating a workspace")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workspace_create() {
  ...
  if [[ -n "$from_workspace" ]]; then
    if ! parent_row="$(workspace_row_by_id "$from_workspace")"; then
      emit_tumux_error "workspace.create"
      return
    fi
  ...
  if ! out="$(tumux_ok_json "${args[@]}")"; then
    ...
    if workspace_create_needs_initial_commit "$err_code" "$err_message"; then
      local retry_cmd
      retry_cmd="skills/tumux/scripts/openclaw-dx.sh workspace create --name $(shell_quote "$name") --project $(shell_quote "$project") --scope $(shell_quote "$scope") --assistant $(shell_quote "${assistant:-codex}")"
      if [[ -n "$from_workspace" ]]; then
        retry_cmd="skills/tumux/scripts/openclaw-dx.sh workspace create --name $(shell_quote "$name") --from-workspace $(shell_quote "$from_workspace") --scope $(shell_quote "$scope") --assistant $(shell_quote "${assistant:-codex}")"
      fi
      if [[ -n "$base" ]]; then
        retry_cmd+=" --base $(shell_quote "$base")"
      fi
    ...
    emit_tumux_error "workspace.create" "$err_payload"
    return
  fi
  ...
  RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$ws_id") --assistant $(shell_quote "${assistant_out:-codex}") --prompt \"Analyze the biggest debt item and implement the fix.\""

  local actions='[]'
  actions="$(append_action "$actions" "start" "Start" "$RESULT_SUGGESTED_COMMAND" "success" "Start a coding turn in this workspace")"
  actions="$(append_action "$actions" "term" "Terminal" "skills/tumux/scripts/openclaw-dx.sh terminal run --workspace $(shell_quote "$ws_id") --text \"pwd\" --enter" "primary" "Run a terminal command in this workspace")"
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$ws_id")" "primary" "Show workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workspace_list() {
  ...
  if ! ws_out="$(tumux_ok_json "${ws_args[@]}")"; then
    emit_tumux_error "workspace.list"
    return
  fi
  ...
  if ! terminals_out="$(tumux_ok_json terminal list)"; then
    terminals_json='[]'
  else
  ...
  if [[ -n "$first_ws" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$first_ws") --assistant codex --prompt \"Summarize current objectives and pick the next coding task.\""
  fi
  ...
  if [[ -n "$first_ws" ]]; then
    ...
    actions="$(append_action "$actions" "status" "Status #1" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$first_ws")" "primary" "Check status for the first listed workspace")"
  fi
  local list_cmd_base="skills/tumux/scripts/openclaw-dx.sh workspace list --limit $limit"
  if [[ -n "$project" ]]; then
  ...
  actions="$(append_action "$actions" "global" "Global" "skills/tumux/scripts/openclaw-dx.sh status" "primary" "Show global coding status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workspace_decide() {
  ...
  if [[ -n "$from_workspace" ]]; then
    if ! parent_row="$(workspace_row_by_id "$from_workspace")"; then
      emit_tumux_error "workspace.decide"
      return
    fi
    if [[ -z "${parent_row// }" ]]; then
  ...
  project_create_cmd="skills/tumux/scripts/openclaw-dx.sh workspace create --name $(shell_quote "$final_project_name") --project $(shell_quote "$project") --assistant $(shell_quote "$assistant")"
  if [[ -n "$parent_id" ]]; then
    nested_create_cmd="skills/tumux/scripts/openclaw-dx.sh workspace create --name $(shell_quote "$name") --from-workspace $(shell_quote "$parent_id") --scope nested --assistant $(shell_quote "$assistant")"
  else
    nested_create_cmd=""
  fi
  ...
  if [[ "$recommendation" == "nested" && -n "$parent_id" ]]; then
    recommended_command="skills/tumux/scripts/openclaw-dx.sh workflow kickoff --from-workspace $(shell_quote "$parent_id") --scope nested --name $(shell_quote "$name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
    alternate_command="skills/tumux/scripts/openclaw-dx.sh workflow kickoff --project $(shell_quote "$project") --name $(shell_quote "$final_project_name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
  else
    recommended_command="skills/tumux/scripts/openclaw-dx.sh workflow kickoff --project $(shell_quote "$project") --name $(shell_quote "$final_project_name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
    if [[ -n "$parent_id" ]]; then
      alternate_command="skills/tumux/scripts/openclaw-dx.sh workflow kickoff --from-workspace $(shell_quote "$parent_id") --scope nested --name $(shell_quote "$name") --assistant $(shell_quote "$assistant") --prompt $(shell_quote "$kickoff_prompt")"
    fi
  fi
...
cmd_continue() {
  ...
  if [[ -z "$agent" && -z "$workspace" ]]; then
    ...
    if active_agents_out="$(tumux_ok_json agent list)"; then
      ...
      if [[ "$active_count" == "1" ]]; then
        ...
        if [[ -n "$first_agent" ]]; then
          RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$first_agent") --text \"Continue from current state and report status plus next action.\" --enter"
        elif [[ -n "$first_workspace" ]]; then
          RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh continue --workspace $(shell_quote "$first_workspace") --text \"Continue from current state and report status plus next action.\" --enter"
        fi
        ...
        while IFS= read -r row; do
          ...
          continue_cmd="skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$row_agent") --text \"Continue from current state and report status plus next action.\" --enter"
        ...
        actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status" "primary" "See all active agents and alerts")"
        RESULT_QUICK_ACTIONS="$actions"
  ...
  if [[ -z "$agent" && -n "$workspace" ]]; then
    ...
    if [[ -z "$agent" ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh continue --workspace $(shell_quote "$workspace") --auto-start --assistant codex --text \"Resume work and provide status plus next action.\""
      RESULT_DATA="$(jq -cn --arg workspace "$workspace" '{workspace: $workspace, reason: "no_active_agent"}')"
      local start_cmd
      start_cmd="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace") --assistant codex --prompt \"Resume work and provide status plus next action.\""
...
cmd_status() {
  ...
  if ! projects_out="$(tumux_ok_json project list)"; then
    emit_tumux_error "$result_command"
    return
  fi
  ...
  if ! agents_out="$(tumux_ok_json "${agents_args[@]}")"; then
    emit_tumux_error "$result_command"
    return
  fi
  ...
  if ! terms_out="$(tumux_ok_json "${term_args[@]}")"; then
    emit_tumux_error "$result_command"
    return
  fi

  if ! sessions_out="$(tumux_ok_json session list)"; then
    emit_tumux_error "$result_command"
    return
  fi

  if ! prune_out="$(tumux_ok_json session prune --older-than "$older_than")"; then
  ...
  while IFS= read -r session_name; do
    ...
    local capture_out
    if ! capture_out="$(tumux_ok_json agent capture "$session_name" --lines "$capture_lines")"; then
      continue
    fi
  ...
  local refresh_cmd
  refresh_cmd="skills/tumux/scripts/openclaw-dx.sh $result_command"
  if [[ -n "$project" ]]; then
  ...
  if [[ -n "$first_needs_input_agent" ]]; then
    ...
    suggested_command="skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$first_needs_input_agent") --text \"Continue using the safest option and then report status plus next action.\" --enter"
    ...
    if [[ -n "$first_completed_workspace" ]]; then
      suggested_command="skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$first_completed_workspace") --assistant codex"
    elif [[ -n "$first_completed_agent" ]]; then
      suggested_command="skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$first_completed_agent") --text \"Summarize final changes, tests, and remaining risks in 5 bullets.\" --enter"
    fi
  elif [[ "$stale_alert_count" -gt 0 ]]; then
    next_action="Clean stale sessions to reduce noise."
    suggested_command="skills/tumux/scripts/openclaw-dx.sh cleanup --older-than $(shell_quote "$older_than") --yes"
  fi
  ...
  if [[ -n "$first_needs_input_agent" ]]; then
    actions="$(append_action "$actions" "reply" "Reply" "skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$first_needs_input_agent") --text \"Continue using the safest option and report status and blockers.\" --enter" "danger" "Reply to blocked agent")"
  fi
  if [[ "$completed_count" -gt 0 ]]; then
    if [[ -n "$first_completed_workspace" ]]; then
      actions="$(append_action "$actions" "review_done" "Review Done" "skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$first_completed_workspace") --assistant codex" "success" "Review completed workspace changes")"
      actions="$(append_action "$actions" "ship_done" "Ship Done" "skills/tumux/scripts/openclaw-dx.sh git ship --workspace $(shell_quote "$first_completed_workspace") --push" "primary" "Commit and push completed workspace changes")"
    elif [[ -n "$first_completed_agent" ]]; then
      actions="$(append_action "$actions" "summary_done" "Summarize" "skills/tumux/scripts/openclaw-dx.sh continue --agent $(shell_quote "$first_completed_agent") --text \"Summarize final changes, tests, and risks.\" --enter" "primary" "Capture final summary for completed agent")"
    fi
  fi
  if [[ "$stale_alert_count" -gt 0 ]]; then
    actions="$(append_action "$actions" "cleanup" "Cleanup" "skills/tumux/scripts/openclaw-dx.sh cleanup --older-than $(shell_quote "$older_than") --yes" "danger" "Prune stale sessions")"
  fi
  local first_ws
  first_ws="$(jq -r '.[0].id // ""' <<<"$ws_enriched")"
  if [[ -n "$first_ws" ]]; then
    actions="$(append_action "$actions" "continue_ws" "Continue WS" "skills/tumux/scripts/openclaw-dx.sh continue --workspace $(shell_quote "$first_ws") --text \"Status update and next action.\" --enter" "success" "Continue active work in top workspace")"
  fi
...
cmd_terminal_run() {
  ...
  RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh terminal logs --workspace $(shell_quote "$workspace") --lines 120"
  ...
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_terminal_preset() {
  ...
  local out
  if ! out="$(tumux_ok_json terminal run --workspace "$workspace" --text "$launch_cmd" --enter=true)"; then
    emit_tumux_error "terminal.preset"
    return
  fi
  ...
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  actions="$(append_action "$actions" "alerts" "Alerts" "skills/tumux/scripts/openclaw-dx.sh alerts --workspace $(shell_quote "$workspace")" "primary" "Check blockers requiring attention")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_terminal_logs() {
  ...
  local attempt=1
  while true; do
    if out="$(tumux_ok_json terminal logs --workspace "$workspace" --lines "$lines")"; then
      break
    fi
    local err_out err_code err_message
    err_out="$TUMUX_ERROR_OUTPUT"
    if [[ -z "${err_out// }" ]] && [[ -n "${TUMUX_ERROR_CAPTURE_FILE:-}" ]] && [[ -f "$TUMUX_ERROR_CAPTURE_FILE" ]]; then
      err_out="$(cat "$TUMUX_ERROR_CAPTURE_FILE" 2>/dev/null || true)"
    fi
    err_code=""
    err_message=""
    ...
    if { [[ "$err_code" == "not_found" && "$err_message" == *"no terminal session found for workspace"* ]]; } || [[ "$err_out" == *"no terminal session found for workspace"* ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"pwd\" --enter"
      ...
      actions="$(append_action "$actions" "preset" "Preset" "skills/tumux/scripts/openclaw-dx.sh terminal preset --workspace $(shell_quote "$workspace") --kind nextjs" "success" "Start a Next.js dev terminal preset")"
      actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
      RESULT_QUICK_ACTIONS="$actions"
    ...
    emit_tumux_error "terminal.logs"
    return
  done
  ...
  RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh terminal run --workspace $(shell_quote "$workspace") --text \"npm test\" --enter"
  RESULT_DATA="$(jq -cn --argjson result "$(jq -c '.data' <<<"$out")" '{terminal: $result}')"
...
cmd_cleanup() {
  ...
  RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh status"
  RESULT_DATA="$(jq -cn --argjson prune "$(jq -c '.data' <<<"$out")" '{prune: $prune}')"

  local actions='[]'
  if [[ "$dry_run" == "true" && "$total" -gt 0 ]]; then
    actions="$(append_action "$actions" "confirm" "Confirm" "skills/tumux/scripts/openclaw-dx.sh cleanup --older-than $(shell_quote "$older_than") --yes" "danger" "Prune stale sessions now")"
  fi
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status" "primary" "Refresh global status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_git_ship() {
  ...
  if [[ -z "${porcelain// }" ]]; then
    ...
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$workspace")"
    ...
    if [[ "$push" == "true" && "$pushed" == "true" ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh git ship --workspace $(shell_quote "$workspace") --push"
    ...
    if [[ "$push" != "true" && "$suggest_push" == "true" ]]; then
      actions="$(append_action "$actions" "push" "Push" "skills/tumux/scripts/openclaw-dx.sh git ship --workspace $(shell_quote "$workspace") --push" "success" "Push existing commits to remote")"
    fi
    actions="$(append_action "$actions" "review" "Review" "skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$workspace")" "primary" "Run review workflow")"
    actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
    RESULT_QUICK_ACTIONS="$actions"
  ...
  if [[ -z "$message" ]]; then
    message="chore(tumux): update $workspace ($file_count files)"
  fi
  ...
  local actions='[]'
  if [[ "$pushed" != "true" ]]; then
    actions="$(append_action "$actions" "push" "Push" "skills/tumux/scripts/openclaw-dx.sh git ship --workspace $(shell_quote "$workspace") --push" "success" "Push latest commit")"
  fi
  actions="$(append_action "$actions" "review" "Review" "skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$workspace")" "primary" "Run review workflow")"
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_workflow_kickoff() {
  ...
  if [[ -n "$project" ]]; then
    ...
    if ! ensured_project="$(ensure_project_registered "$project")"; then
      emit_tumux_error "workflow.kickoff"
      return
    fi
      ...
      ((.quick_actions // []) + [
        {
          id: "status_ws",
          label: "Status",
          command: ("skills/tumux/scripts/openclaw-dx.sh status --workspace " + $workspace_id),
          style: "primary",
        ...
        },
        {
          id: "review_ws",
          label: "Review",
          command: ("skills/tumux/scripts/openclaw-dx.sh review --workspace " + $workspace_id + " --assistant codex"),
          style: "primary",
...
cmd_workflow_dual() {
  ...
  RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh git ship --workspace $(shell_quote "$workspace")"
  local codex_continue_cmd
  codex_continue_cmd="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace") --assistant codex --prompt \"Continue from current state and provide concise status plus next action.\""
  ...
  if [[ "$impl_ok" != "true" ]]; then
    ...
    if [[ -n "$impl_cmd" ]]; then
      ...
      RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$implement_assistant") --prompt $(shell_quote "$implement_prompt")"
    fi
    ...
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$effective_review_assistant")"
  ...
  if [[ "$review_json" == "null" && "$review_skipped_reason" != "implementation_not_ready" ]]; then
    actions="$(append_action "$actions" "run_review" "Run Review" "skills/tumux/scripts/openclaw-dx.sh review --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$effective_review_assistant")" "primary" "Run review phase now")"
  ...
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status --workspace $(shell_quote "$workspace")" "primary" "Check workspace status")"
  actions="$(append_action "$actions" "alerts" "Alerts" "skills/tumux/scripts/openclaw-dx.sh alerts --workspace $(shell_quote "$workspace")" "primary" "Check blocking alerts")"
  if [[ "$RESULT_STATUS" == "ok" ]]; then
    actions="$(append_action "$actions" "ship" "Ship" "skills/tumux/scripts/openclaw-dx.sh git ship --workspace $(shell_quote "$workspace")" "success" "Commit current changes")"
  fi
  RESULT_QUICK_ACTIONS="$actions"
...
cmd_assistants() {
  local config_path
  config_path="${TUMUX_HOME:-$HOME/.tumux}/config.json"
  local workspace=""
  local probe=false
  ...
  if [[ "$missing_count" -gt 0 ]]; then
    RESULT_NEXT_ACTION="Install or remap missing assistant binaries in ~/.tumux/config.json."
  fi
  if [[ "$probe_needs_input" -gt 0 ]]; then
  ...
  if [[ -n "$workspace" && "$dual_ready" == "true" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh workflow dual --workspace $(shell_quote "$workspace") --implement-assistant claude --review-assistant codex"
  elif [[ -n "$workspace" && -n "$preferred_assistant" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$preferred_assistant") --prompt \"Summarize current status and next action in one line.\""
  elif [[ -n "$workspace" && -n "$first_ready" ]]; then
    RESULT_SUGGESTED_COMMAND="skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$first_ready") --prompt \"Summarize current status and next action in one line.\""
  fi

  local actions='[]'
  actions="$(append_action "$actions" "status" "Status" "skills/tumux/scripts/openclaw-dx.sh status" "primary" "Show current work/agent status")"
  actions="$(append_action "$actions" "review" "Review" "skills/tumux/scripts/openclaw-dx.sh review --workspace <workspace_id> --assistant codex" "primary" "Run a review workflow")"
  if [[ "$probe" != "true" && -n "$workspace" ]]; then
    actions="$(append_action "$actions" "probe" "Probe" "skills/tumux/scripts/openclaw-dx.sh assistants --workspace $(shell_quote "$workspace") --probe --limit $(shell_quote "$limit")" "primary" "Run readiness probes for ready assistants")"
  fi
  if [[ -n "$workspace" && "$dual_ready" == "true" ]]; then
    actions="$(append_action "$actions" "dual" "Dual Pass" "skills/tumux/scripts/openclaw-dx.sh workflow dual --workspace $(shell_quote "$workspace") --implement-assistant claude --review-assistant codex" "success" "Implement with claude and review with codex")"
  elif [[ -n "$workspace" && -n "$preferred_assistant" ]]; then
    actions="$(append_action "$actions" "start_ready" "Start Ready" "skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$preferred_assistant") --prompt \"Summarize current status and next action in one line.\"" "primary" "Start with best probe-passed assistant")"
  elif [[ -n "$workspace" && -n "$first_ready" ]]; then
    actions="$(append_action "$actions" "start_ready" "Start Ready" "skills/tumux/scripts/openclaw-dx.sh start --workspace $(shell_quote "$workspace") --assistant $(shell_quote "$first_ready") --prompt \"Summarize current status and next action in one line.\"" "primary" "Start with first ready assistant")"
  fi
  RESULT_QUICK_ACTIONS="$actions"
...
require_prereqs() {
  ...
  if ! command -v tumux >/dev/null 2>&1; then
    printf '{"ok":false,"command":"unknown","status":"command_error","summary":"tumux is required","error":"missing binary: tumux"}\n'
    exit 0
  fi
}
...
DX_CMD_REF="${OPENCLAW_DX_CMD_REF:-skills/tumux/scripts/openclaw-dx.sh}"
TURN_CMD_REF="${OPENCLAW_DX_TURN_CMD_REF:-skills/tumux/scripts/openclaw-turn.sh}"
STEP_CMD_REF="${OPENCLAW_DX_STEP_CMD_REF:-skills/tumux/scripts/openclaw-step.sh}"
