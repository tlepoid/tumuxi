...
Runs a real OpenClaw/tumux dogfood flow end-to-end:
  - project add
  - start coding
  - continue coding
...
TMP_ROOT=""
if [[ -z "${REPO_PATH// }" ]]; then
  TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/tumux-openclaw-dogfood-script.XXXXXX")"
...
if [[ -z "${REPORT_DIR// }" ]]; then
  REPORT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tumux-openclaw-dogfood-report.XXXXXX")"
  REPORT_DIR_CREATED=true
fi
mkdir -p "$REPORT_DIR"
DX_CONTEXT_FILE="$REPORT_DIR/openclaw-dx-context.json"
CHANNEL_AGENT_CREATED=false
CHANNEL_AGENT_ID="${OPENCLAW_DOGFOOD_OPENCLAW_AGENT:-tumux-dx}"
...
prepare_channel_agent() {
  ...
  base_id="${CHANNEL_AGENT_ID:-tumux-dx}"
...
# AGENTS
- You are a strict terminal command runner for tumux workflows.
...
run_openclaw_channel_command() {
  ...
  local primary_agent="${OPENCLAW_DOGFOOD_OPENCLAW_AGENT:-tumux-dx}"
