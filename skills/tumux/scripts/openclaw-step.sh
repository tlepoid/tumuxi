#!/usr/bin/env bash
# openclaw-step.sh — Bounded tumux step runner for chat/orchestrator flows.
#
# Usage:
...
STEP_SCRIPT_REF="${OPENCLAW_STEP_CMD_REF:-skills/tumux/scripts/openclaw-step.sh}"
...
is_chrome_line() {
  local line="$1"
  case "$line" in
    ""|"|"|✻|"╭"*|"╰"*|"│"*|"─"*|"└ "*|"⎿ "*|"↳ Interacted with "*|"› "*|"❯ "*|"? for shortcuts"*|"✶ "*|"✻ "*|"▟"*|"▐"*|"▝"*|"▘"*|"Tip:"*|"model:"*|"directory:"*|"cwd:"*|"workspace:"*|"• Explored"|"• Exploring"|"• Working ("*|"Working ("*|"Thinking "*|*" no sandbox "*|*"/model "*|"~/.tumux/"*|*"sandbox   "*|*"sandbox "*")"|"shift+tab to accept edits"*|"/ commands · @ files · ! shell"*|*"? for help"*|*"▄▄▄▄"*|*"███"*|*"▀▀▀"*|">   Type your message or @path/to/file"*)
      return 0
      ;;
    *)
      return 1
...
if ! command -v tumux >/dev/null 2>&1; then
  print_json_error "$MODE" "command_error" "tumux is not installed" "missing binary: tumux"
  exit 127
fi
...
if [[ "$COMMAND_TIMED_OUT" == "true" ]]; then
  detail="hard timeout (${HARD_TIMEOUT_SECONDS}s) exceeded while running tumux step"
  if [[ -n "${RAW_OUTPUT// }" ]]; then
    detail="$detail"$'\n'"$RAW_OUTPUT"
  fi
  print_json_error "$MODE" "command_error" "tumux command exceeded hard timeout" "$detail"
  exit 124
fi

if [[ $CMD_EXIT -ne 0 ]]; then
  print_json_error "$MODE" "command_error" "tumux command failed" "$RAW_OUTPUT"
  exit "$CMD_EXIT"
fi

if ! jq -e . >/dev/null 2>&1 <<<"$RAW_OUTPUT"; then
  print_json_error "$MODE" "command_error" "tumux returned non-JSON output" "$RAW_OUTPUT"
  exit 65
fi
...
if [[ "$STATUS" == "timed_out" && "$SUBSTANTIVE_OUTPUT" != "true" && -n "$SESSION_NAME" ]]; then
  ...
  for ((i=1; i<=RECOVERY_POLLS; i++)); do
    ...
    capture_json="$(tumux --json agent capture "$SESSION_NAME" --lines "$RECOVERY_LINES" 2>/dev/null || true)"
